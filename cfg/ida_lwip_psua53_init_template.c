...
void initializeNetwork(void) {
	...
	EthDev.hwaddr_len = 6;
	for (int i = 0; i < 6; i++)
		EthDev.hwaddr[i] = macBoard1.addr[i];

	if(!netif_add(&EthDev, &ipaddrBoard1, &netmask, &ipaddrGW,NULL,ida_lwip_virtEth_init,NULL)){
		return;
	}
#elif BOARD == 2
	EthDev.hwaddr_len = 6;
	for (int i = 0; i < 6; i++)
		EthDev.hwaddr[i] = macBoard2.addr[i];

	if(!netif_add(&EthDev, &ipaddrBoard2, &netmask, &ipaddrGW,NULL,ida_lwip_virtEth_init,NULL)){
		return;
	}
#else
#error "Please specify board number"
	...
	sys_thread_new("virtEth_input_thread",(void (*)(void*)) ida_lwip_virtEth_rxTask, &EthDev, 512,	TCPIP_THREAD_PRIO + 1);
}
